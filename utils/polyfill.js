...
(function(root, factory) {
  ...
  defineProperties(ArrayPrototype, {
    forEach: function forEach(callbackfn/*, thisArg*/) {
      ...
      var self = splitString && isString(this)
        ? strSplit(this, '')
        : object;
      var i = -1;
    ...
    defineProperties(ArrayPrototype, {
      join: function join(separator) {
        var sep = typeof separator === 'undefined'
          ? ','
          : separator;
        return originalJoin.call(isString(this)
          ? strSplit(this, '')
          : this, sep);
      }
    }, hasStringJoinBug);
  ...
  defineProperties(Date.prototype, {
    toUTCString: function toUTCString() {
      ...
      return dayName[day] + ', ' + (date < 10
        ? '0' + date
        : date) + ' ' + monthName[month] + ' ' + year + ' ' + (hour < 10
        ? '0' + hour
        : hour) + ':' + (minute < 10
        ? '0' + minute
        : minute) + ':' + (second < 10
  ...
  defineProperties(Date.prototype, {
    toDateString: function toDateString() {
      ...
      return dayName[day] + ' ' + monthName[month] + ' ' + (date < 10
        ? '0' + date
        : date) + ' ' + year;
    }
  ...
  if (hasNegativeMonthYearBug || hasToStringFormatBug) {
    Date.prototype.toString = function toString() {
      ...
      return dayName[day] + ' ' + monthName[month] + ' ' + (date < 10
        ...
        : minute) + ':' + (second < 10
        ? '0' + second
        : second) + ' GMT' + (timezoneOffset > 0
        ? '-'
        : '+') + (hoursOffset < 10
        ? '0' + hoursOffset
        : hoursOffset) + (minutesOffset < 10
        ? '0' + minutesOffset
        : minutesOffset);
    };
    if (supportsDescriptors) {
  ...
  defineProperties(Date.prototype, {
    toISOString: function toISOString() {
      ...
      year = ((year < 0
        ...
        : (year > 9999
          ? '+'
          : '')) + strSlice('00000' + Math.abs(year), (0 <= year && year <= 9999)
        ? -4
        : -6));
    ...
    Date = (function(NativeDate) {
                  ...
                  : length >= 4
                    ...
                    : length >= 3
                      ...
                      : length >= 2
                        ? new NativeDate(Y, M)
                        : length >= 1
                          ? new NativeDate(Y instanceof NativeDate
                            ? + Y
                            : Y)
                          : new NativeDate();
        } else {
      ...
      var parseShim = function parse(string) {
        ...
        if (match) {
          ...
          if (hour < (hasMinutesOrSecondsOrMilliseconds
            ? 24
            : 25) && minute < 60 && second < 60 && millisecond < 1000 && month > -1 && month < 12 && hourOffset < 24 && minuteOffset < 60 && // detect invalid offsets
  ...
  defineProperties(StringPrototype, {
    lastIndexOf: function lastIndexOf(searchString) {
      ...
      var numPos = arguments.length > 1
        ? $Number(arguments[1])
        : NaN;
      var pos = isActualNaN(numPos)
        ? Infinity
        : ES.ToInteger(numPos);
